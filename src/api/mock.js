  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      if (params.online !== undefined) {
        filteredPlaymates = filteredPlaymates.filter(p => p.isOnline === params.online)
      }

      if (params.priceRange) {
        const [min, max] = params.priceRange.split('-').map(Number)
        if (params.priceRange === '100+') {
          filteredPlaymates = filteredPlaymates.filter(p => p.price >= 100)
        } else {
          filteredPlaymates = filteredPlaymates.filter(p => p.price >= min && p.price <= max)
        }
      }

      if (params.rank) {
        filteredPlaymates = filteredPlaymates.filter(p => p.rank === params.rank)
      }

      if (params.gender) {
        filteredPlaymates = filteredPlaymates.filter(p => p.gender === params.gender)
      }

      if (params.keyword) {
        const keyword = params.keyword.toLowerCase()
        filteredPlaymates = filteredPlaymates.filter(p => 
      ...
      if (params.sortBy) {
        switch (params.sortBy) {
          case 'rating':
            filteredPlaymates.sort((a, b) => b.rating - a.rating)
            break
          case 'price_asc':
            filteredPlaymates.sort((a, b) => a.price - b.price)
            break
          case 'price_desc':
            filteredPlaymates.sort((a, b) => b.price - a.price)
            break
          case 'newest':
            filteredPlaymates.reverse()
            break
        }
      }
      ...
      const paginatedData = filteredPlaymates.slice(startIndex, endIndex)
      ...
      resolve({
        ...
        pagination: {
          currentPage: page,
          totalPages: Math.ceil(filteredPlaymates.length / pageSize),
          totalCount: filteredPlaymates.length
        }
      })
    }, 300)
  })
...
export function mockGetSearchSuggestions(keyword) {
  return new Promise((resolve) => {
    setTimeout(() => {
      const filtered = mockSearchSuggestions.filter(s => 
...
export function mockGetBrowseHistory() {
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve({
        success: true,
        data: mockPlaymates.slice(0, 5).map(p => ({
          ...p,
          viewedAt: '2026-03-23'
        }))
      })
...
export function mockGetCommunityPosts(params = {}) {
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      if (params.category && params.category !== 'hot') {
        const categoryMap = {
          'game': '游戏',
          'mood': '心情',
          'social': '交友',
        ...
        filteredPosts = filteredPosts.filter(post => 
          post.content.includes(categoryMap[params.category]) || 
          post.game === categoryMap[params.category]
        )
      }

      if (params.keyword) {
        filteredPosts = filteredPosts.filter(post => 
          post.content.toLowerCase().includes(params.keyword.toLowerCase()) ||
          post.user.name.toLowerCase().includes(params.keyword.toLowerCase())
        )
      }
...
export function mockGetGamesByCategory(category = 'moba') {
  return new Promise((resolve) => {
    setTimeout(() => {
      const filteredGames = mockGames.filter(game => game.category === category)
      resolve({
        success: true,
        data: filteredGames
      })
...
export function mockGetOrderConfirmation(orderId) {
  return new Promise((resolve) => {
    setTimeout(() => {
      const expert = mockPlaymates[0] // 固定使用第一个专家作为示例
      resolve({
        success: true,
        data: {
          orderId: orderId,
        ...
        data: [
          {
            ...
            status: 'completed',
            statusText: '已到账',
            createdAt: '2026-03-22 15:30',
            completedAt: '2026-03-22 16:00'
          },
          {
            id: 2,
            amount: 1000.00,
